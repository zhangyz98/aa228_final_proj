...
class NN(tf.keras.Model):
    def __init__(self, in_size, out_size):
        ...
        self.W3 = tf.Variable(tf1.contrib.layers.xavier_initializer()(shape=[32, out_size]))
        self.b3 = tf.Variable(tf.zeros([out_size, ]))
    ...
    def call(self, x):
        ...
        a2 = tf1.nn.tanh(y2)
        y3 = tf.add(tf.matmul(a2, self.W3), self.b3)
        a3 = tf1.nn.tanh(y3)
        # y4 = tf.add(tf.matmul(a3, self.W4), self.b4)
        y_est = a3
...
def loss(y_est, y):
    ...
    steer_loss = 5 * tf.reduce_mean((y_est[:, 0] - y[:, 0]) ** 2)
