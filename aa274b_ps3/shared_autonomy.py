...
if __name__ == '__main__':
    ...
    for _ in range(10):
        ...
        while not done:
            ...
            for goal_id in range(len(goals[scenario_name])):
                goal = goals[scenario_name][goal_id]
                E_aH = np.array(nn_models[goal].call(obs)[:, 0:2]).reshape(1, 2)
                a_robot += np.average(scores[-11:, goal_id], axis=0) * (optimal_action[goal] - E_aH)
            a_robot[0, 0] = np.clip(a=a_robot[0, 0], a_max=max_steering, a_min=None)
            a_robot[0, 1] = np.clip(a=a_robot[0, 1], a_max=max_throttle, a_min=None)
            ...
            probs = []
            P_go = 1. / len(goals[scenario_name])
            P_ao = 0
            P_aog = np.zeros((len(goals[scenario_name])))
            for goal_idx in range(len(goals[scenario_name])):
                goal = goals[scenario_name][goal_idx]
                y_est = np.array(nn_models[goal].call(obs)).reshape(-1)
                mu = y_est[0:2]
                tril = np.zeros((len(mu), len(mu)))
                tril_idx = np.tril_indices_from(tril, 0)
                tril_elem = y_est[2:]
                tril[tril_idx] = tril_elem[::-1]
                cov = tril.dot(tril.T)
                P_aog[goal_idx] = multivariate_normal.pdf(x=a_human, mean=mu, cov=cov)

            P_ao = np.sum(P_aog * P_go)
            for goal_idx in range(len(goals[scenario_name])):
                probs.append(P_aog[goal_idx] * P_go / P_ao)
