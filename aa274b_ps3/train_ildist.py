...
class NN(tf.keras.Model):
    def __init__(self, in_size, out_size):
        ...
        out_size = tf.cast(out_size + out_size * (out_size + 1) / 2, dtype=tf.int32)  # 5
    ...
    def call(self, x):
        ...
        # a2 = tf1.nn.tanh(y2)
        y3 = tf.add(tf.matmul(a1, self.W3), self.b3)
        a3 = tf1.nn.tanh(y3)
        y_est = a3
...
def loss(y_est, y):
    ...
    mu = y_est[:, 0:2]
    L = tfd.fill_triangular(y_est[:, 2:])
    cov = tf.matmul(L, tf.transpose(L, perm=[0, 2, 1])) + tf.multiply(1e-6, tf.eye(2))
    mvn = tfd.MultivariateNormalFullCovariance(loc=mu, covariance_matrix=cov)
    prob = mvn.log_prob(y)
    l = - tf.reduce_mean(prob[:])
