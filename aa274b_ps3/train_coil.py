...
class NN(tf.keras.Model):
    def __init__(self, in_size, out_size):
        ...
        self.W1 = tf.Variable(tf1.contrib.layers.xavier_initializer()(shape=[in_size, 32]))
        self.b1 = tf.Variable(tf.zeros([32, ]))
        self.W2 = tf.Variable(tf1.contrib.layers.xavier_initializer()(shape=[32, 32]))
        self.b2 = tf.Variable(tf.zeros([32, ]))
        self.W3_branch0 = tf.Variable(tf1.contrib.layers.xavier_initializer()(shape=[32, out_size]))
        self.b3_branch0 = tf.Variable(tf.zeros([out_size, ]))
        self.W3_branch1 = tf.Variable(tf1.contrib.layers.xavier_initializer()(shape=[32, out_size]))
        self.b3_branch1 = tf.Variable(tf.zeros([out_size, ]))
        self.W3_branch2 = tf.Variable(tf1.contrib.layers.xavier_initializer()(shape=[32, out_size]))
        self.b3_branch2 = tf.Variable(tf.zeros([out_size, ]))
        # self.W4 = tf.Variable(tf1.contrib.layers.xavier_initializer()(shape=[32, out_size]))
        # self.b4 = tf.Variable(tf.zeros([out_size, ]))
    ...
    def call(self, x, u):
        ...
        y1 = tf.add(tf.matmul(x, self.W1), self.b1)
        a1 = tf1.nn.tanh(y1)
        y2 = tf.add(tf.matmul(a1, self.W2), self.b2)
        a2 = tf1.nn.tanh(y2)
        y3_0 = tf.add(tf.matmul(a2, self.W3_branch0), self.b3_branch0)
        a3_0 = tf1.nn.tanh(y3_0)
        y3_1 = tf.add(tf.matmul(a2, self.W3_branch1), self.b3_branch0)
        a3_1 = tf1.nn.tanh(y3_1)
        y3_2 = tf.add(tf.matmul(a2, self.W3_branch2), self.b3_branch0)
        a3_2 = tf1.nn.tanh(y3_2)

        y_est = tf.zeros_like(a3_0)
        y_est = tf.where(tf.math.equal(u, 0), a3_0, y_est)
        y_est = tf.where(tf.math.equal(u, 1), a3_1, y_est)
        y_est = tf.where(tf.math.equal(u, 2), a3_2, y_est)
...
def loss(y_est, y):
    ...
    steer_loss = 1 * tf.reduce_mean((y_est[:, 0] - y[:, 0]) ** 2)
    throttle_loss = 1 * tf.reduce_mean((y_est[:, 1] - y[:, 1]) ** 2)
    total_loss = steer_loss + throttle_loss
    return total_loss
